...
class GetChinaMsg():
    def __init__(self):
        ...
        初始化邮件正文的商品名称
        """
        self.excl_chain_conn, self.excl_chain_conn1, self.excl_chain_cur1, self.excl_chain_cur = sqlFile_test.excl_chain()
    ...
    def GetMsg(self):
        ...
        try:
            self.excl_chain_cur1.execute(sql1)

        except Exception as e:
            print('查询错误：{}'.format(e))

        SkuList = self.excl_chain_cur1.fetchall()
        ...
        for i in range(len(ShoesList)):
            ...
            if productId not in OldPublishTime and ShoesSku not in ['BQ4800-100', 'CD9329-001', 'AV4052-100', 'CK1905-100', 'CK1907-600', 'AO3189-001', 'AO3189-100']:
                ...
                if 'title' in ShoesList[i]['product'] and restricted is True:
                    ...
                    sql_1 = """INSERT INTO monitor_result (title, sku, distributionchannels, replenishmenttype, pushtime, picurl, size, `status`, createtime, distributionid, linkurl, productid) VALUES("{}", "{}", 'SNKRS中国', '{}', now(), '{}', "{}", 3, now(), 1, '{}', '{}')""".format(title, ShoesSku, Additional_information, Img_url, replenishment_dict,TheLinkadDress, productId)

                    try:
                        self.excl_chain_cur1.execute(sql_1)

                    except Exception as e:
                        print('插入错误：{}'.format(e))

                    self.excl_chain_conn1.commit()
                    ...
                    try:
                        self.excl_chain_cur.execute(sql_7)
                    except Exception as E:
                        print(E)

                    Grab_judgment = self.excl_chain_cur.fetchall()
        ...
        if len(ShoeTitle) > 0:
            self.excl_chain_cur.close()
            self.excl_chain_conn.close()

            self.excl_chain_cur1.close()
            self.excl_chain_conn1.close()
            print('发送保存成功')
