...
class GetChinaMsg():
    def __init__(self):
        ...
        初始化邮件正文的商品名称
        """
        self.Pass_Uk_conn, self.Pass_Uk_conn1, self.Pass_Uk_cur1, self.Pass_Uk_cur = sqlFile_test.Pass_Uk()
            ...
            if ShoesList[i]['id'] not in OldPublishTime:
                ...
                if 'title' not in ShoesList[i]['product']:
                    ...
                    if ShoesSku not in ['BQ4800-100', 'CD9329-001', 'AV4052-100', 'CK1905-100', 'CK1907-600', 'AO3189-001', 'AO3189-100']:
                        ...
                        sql_1 = """INSERT INTO monitor_result (title, sku, distributionchannels, replenishmenttype, pushtime, picurl, `status`, createtime, distributionid, linkurl, productid) VALUES("{}", "{}", 'SNKRS英国', '{}', '{}', '{}', 2, now(), 3, '{}', '{}')""".format(title, ShoesSku, Additional_information, startSellDate, Img_url, TheLinkadDress, productId)
