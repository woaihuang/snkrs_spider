...
class GetChinaMsg():
    ...
    def GetHtml(self):
        ...
        while lensnkrs < 200:
            reql = requests.get('{}'.format('https://api.nike.com/snkrs/content/v1/?&country=JP&language=ja&offset=0&orderBy=published'))
                    ...
                    if len(skustr) > 1:
                        ...
                        if len(Grab_judgment) == 0:
                            ...
                            sql = """INSERT INTO monitor_result (title, sku, distributionchannels, replenishmenttype, picurl, size, `status`, createtime, distributionid, linkurl, productid, sortnum, pushid)
                                                    VALUES("{}", '{}', 'SNKRS日本', '{}', '{}', "{}", 1, now(), 4, '{}', '{}', 3, {})""".format(title, ShoesSku, Additional_information, Img_url, replenishment_dict, TheLinkadDress, productId, pushid)
